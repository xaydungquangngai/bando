...
(function($){
	$.fn.menu = function(options)
	{
		...
		this.find( '*[menu_method=new]' ).on('click',
            function(){
    			if(confirm( 'Thông tin hiện đang được chỉnh sửa sẽ bị loại bỏ. Là nó ổn?' )){
...
(function($){
	$.fn.loadDialog = function(options){
		...
		container.find("*[load_method=load]").on('click',
            function(){
			    if(_fileCheck()){
				    ...
				    alert( 'Chọn tệp có phần mở rộng .txt .json' );
			    }
		    }
        );
		...
		var _load = function(file){
			...
			reader.onerror = function(){ alert( 'Không thể đọc tệp' ); };
		...
		var _onFileLoad = function(){
			...
			try{
				...
				alert( 'Có lỗi trong phần mô tả của tệp cấu hình' );
			}
		...
		// chuyển đổi sang định dạng nội bộ
...
(function($){
	...
	var _createTreeItem = function(o, treeObject, item){
		...
		var titleText = (item.title && item.title != '' ? '<span style="color:#222;">' +item.title + '</span>' : '<span style="color:#999;">không có tiêu đề</span>' );
			...
			o.oEdit.edit("set", o, $(target).data( '_data' ), 
                {
				    onChange : function(target,data){
					    ...
					    var titleText = (_data.title && _data.title != '' ? _data.title : '<span style="color:#999;">không có tiêu đề</span>');
		...
		exec : function(o, methodName, target){
			...
			if(targetFolderElem){
				...
				if(item){
					...
					var title = (item.title && item.title != '' ? item.title : '<span style="color:#999;">không có tiêu đề</span>');
            ...
            function() {
			    ...
			    a.attr( { href:'javascript:void(0);', target:''} ).on('click',
                    function(){
                        ...
                        if(o.fSaveMsg){
                            f = false;
                            if(window.confirm("Các phần tử trong thư mục chứa URL của tệp định nghĩa lớp sẽ không được lưu.")){
                                f = true;
                            }
                        }
                        if(f){
