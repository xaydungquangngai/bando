...
function InitProgressMsgInfo(msg) {
	console.log("Bản đồ 3D[thông tin]>" + msg);
};

function InitProgressMsgError(msg) {
	console.log("Bản đồ 3D[lỗi]>" + msg);
};
	...
	if (o != null) {
		...
		oFrame3D = document.createElement("div"); oFrame.appendChild(oFrame3D);
		{
			...
			oFrame3D_CtrlZ.innerHTML =
				"<div style=\"vertical-align  :middle;font-size:10pt;\"><div style=\"float:left; font-size:10pt;margin-right:4px;margin-bottom:3px;\">Độ phóng đại theo chiều cao=<input type=\"text\" id=\"ratioZ\" value=\"1.0\" style=\"width:2em;\" onChange=\"SceneGeometryZ_Value();\"/></div>"
...
function InitFrameDownload(o) {
	if (o != null) {
		...
		oFrame3D_Download.innerHTML = ""
			...
			+ "<tbody>"
			+ "<tr>"
			+ "<th style=\"font-weight: bold;\">tập tin STL</th>"
			+ "<td>Dữ liệu cho máy in 3D không thể tô màu</td>"
			+ "<td><input id=\"dl_stl\" onclick=\"showDownloadWindow('stl');\"     type=\"button\" value=\"Tải xuống\"></td>"
			+ "</tr>"
			+ "<tr>"
			+ "<th style=\"font-weight: bold;\">tập tin VRML</th>"
			+ "<td>Dữ liệu cho máy in 3D đủ màu</td>"
			+ "<td><input id=\"dl_vrml\" onclick=\"showDownloadWindow('vrml');\"   type=\"button\" value=\"Tải xuống\"></td>"
			+ "</tr>"
			+ "<tr>"
			+ "<th style=\"font-weight: bold;\">tập tin WebGL</th>"
			+ "<td>Nó là một tệp để quay vòng trong trình duyệt (tệp màn hình hiện tại)</td>"
			+ "<td><input id=\"dl_three\" onclick=\"showDownloadWindow('webgl');\" type=\"button\" value=\"Tải xuống\"></td>"
			+ "</tr>"
			+ "<tr>"
			+ "<th style=\"font-weight: bold;\">tập tin OBJ</th>"
			+ "<td>định dạng có thể được mở bởi nhiều ứng dụng</td>"
			+ "<td><input id=\"dl_three\" onclick=\"showDownloadWindow('obj');\" type=\"button\" value=\"Tải xuống\"></td>"
			+ "</tr>"
			+ "</tbody>"
			+ "</table>"
		...
		oGSIMapLink = $("<a>").attr({
			id: "gsimap_link",
			"target": "_blank"
		}).html("Hiển thị trên bản đồ");


		oHelpPanel = $("<div>").attr({
			id: "help-panel"
		}).html(
			'<div class="line"><span>phóng:</span>con lăn chuột</div>' +
			'<div class="line"><span>quay:</span>kéo chuột trái</div>' +
			'<div class="line"><span>Thay đổi trục quay:</span>kéo chuột phải</div>'
		);
		oHelpLink = $("<a>").attr({
			id: "gsimap_help",
			"href": "javascript:void(0);"
		}).html("giải thích hoạt động").on('click',function () {
...
function InitLoadLayers() {
	...
	oLayersTM = setInterval(function () {
		if (vLayersTM_Cur >= vLayersTM_Max) {
			clearInterval(oLayersTM);
			InitProgressMsgInfo("Hết giờ([" + (vLayersTM_Max / 1000) + "]giây)");
		}
		else {
			vLayersTM_Cur += vLayersTM;
...
CanvasRenderingContext2D.prototype.fillTextVertical = function (text, x, y, deg) {
	...
	CanvasRenderingContext2D.prototype._Text = function (text) {
		...
		_CL.push(this._TextData(_CL, "Plane", "　"));
		_CL.push(this._TextData(_CL, "Plane", "xin lỗi"));
		_CL.push(this._TextData(_CL, "Plane", "Tôi không thể chờ đợi tôi xin lỗi"));
		_CL.push(this._TextData(_CL, "Plane", "＝≠＜＞≦≧∈∋⊆⊇⊂⊃∪∩⊄⊅⊊⊋∉⌅⌆∧∨⇒⇔∥∦≡≒≪≫∽∝≢≃≅≈≶≷⊥↔⋚⋛"));
		_CL.push(this._TextData(_CL, "Plane", "＋－±×÷⊕⊖⊗∓"));
		_CL.push(this._TextData(_CL, "Plane", " "));
		_CL.push(this._TextData(_CL, "Bracket", "（〔［", 1.3));
